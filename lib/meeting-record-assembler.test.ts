...
describe('buildMeetingRecord — RT-A1/A2: full-form vs fast-path parity for otherwise-equivalent input', () => {
  it('produces the same identity/attribution/companion shape for equivalent full-form and fast-path input', () => {
    const full = buildMeetingRecord({
      ...
      remarks: '',
      poEvidence: null,
      endGps: { capturedAt: '2026-08-02T09:30:00.000Z', gpsLat: 14.6, gpsLng: 120.99 },
    });
    ...
    // F-003 gap fix: full-form now carries end GPS too, same shape as fast-path.
    expect(full.end_gps_lat).toBe(visit.end_gps_lat);
    expect(full.end_gps_lng).toBe(visit.end_gps_lng);
  });
...
describe('buildMeetingRecord — F-003 gap fix: full-form end GPS parity with fast-path', () => {
  it('full-form sets end_gps_lat/end_gps_lng/end_captured_at from endGps when the Save-time fix succeeded', () => {
    const record = buildMeetingRecord({
      ...COMMON,
      flow: 'full',
      selfieUri: 'file://selfie.jpg',
      ...
      remarks: '',
      poEvidence: null,
      endGps: { capturedAt: '2026-08-09T10:15:00.000Z', gpsLat: 14.7, gpsLng: 121.1 },
    });
    expect(record.end_gps_lat).toBe(14.7);
    expect(record.end_gps_lng).toBe(121.1);
    expect(record.end_captured_at).toBe('2026-08-09T10:15:00.000Z');
    // Start attribution must remain untouched by the end capture (RT-A5).
    expect(record.gps_lat).toBe(START.gpsLat);
    expect(record.gps_lng).toBe(START.gpsLng);
  });

  it('full-form falls back to null end GPS fields (never blocks the record shape) when endGps is null', () => {
    const record = buildMeetingRecord({
      ...COMMON,
      flow: 'full',
      selfieUri: 'file://selfie.jpg',
    ...
    expect(record.end_gps_lat).toBeNull();
    expect(record.end_gps_lng).toBeNull();
    expect(record.end_captured_at).toBeNull();
  });
});
