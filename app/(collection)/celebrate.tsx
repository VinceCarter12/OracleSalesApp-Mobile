...
/**
 * F-007 first draft (2026-07-25): payment success — wireframe `c-celebrate`.
 * Reached via router.replace from the Collect Payment screen (so Back doesn't
 * return to the half-filled form). Continue pops the whole stack back to the
 * dashboard, which re-reads the now-decremented mock store list.
 */
export default function CollectCelebrateScreen() {
        ...
        <YStack alignItems="center" gap="$4" width="100%">
          ...
          <XStack alignItems="center" gap="$2" backgroundColor={BIZLINK_ON_INK.circleFill} borderRadius={999} paddingHorizontal={18} paddingVertical={9}>
            ...
            <Text fontSize={12.5} fontFamily={BIZLINK_FONTS.medium} color={BIZLINK_ON_INK.solid}>
              Queued for sync — SMS confirmation to follow
            </Text>
          </XStack>
          <YStack width="100%" marginTop={26}>
            <BizButton label="Continue" variant="white" onPress={done} />
          </YStack>
        </YStack>
      </ScrollView>
    </YStack>
