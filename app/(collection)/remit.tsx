...
import { formatPeso, getCollectionSummary, COLLECTION_STORES } from '../../lib/collection-delivery-data';

/**
 * F-007 first draft (2026-07-25): Remit Collections — wireframe `c-remit`.
 * Office remittance needs an assigned receiver + signed-proof photo + the
 * receiver's digital signature (F-007, 2026-07-16); 7-11/bank needs just the
 * receipt photo. Submit is UI-only for now — no schema/outbox entity exists
 * yet (Database.md "Planned schema notes — F-007"), so nothing persists.
 */
...
export default function CollectionRemitScreen() {
  ...
  // Signature strokes live inside SignaturePad; this key remounts it (and so
  ...
  const canSubmit = isOffice
    ? receiverId !== null && officeProofUri !== null && signed
    : outsideProofUri !== null;

  function submit(): void {
    Alert.alert(
      'Remittance recorded',
      `${formatPeso(summary.forRemittance)} — ${isOffice ? 'Office' : destination === '711' ? '7-11 / Bayad center' : 'Bank deposit'}\n\nFirst draft: UI lang muna — hindi pa ito nase-save o nasi-sync (walang F-007 schema pa).`
    );
    setReceiverId(null);
